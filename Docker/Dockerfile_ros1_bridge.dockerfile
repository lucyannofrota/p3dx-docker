...
## FOXY
FROM jetson:noetic as noetic-foxy
...
ARG BASE_IMAGE=lucyannofrota/jetson:noetic
ENV BASE_IMAGE=${BASE_IMAGE}
ARG IMAGE_NAME=jetson:noetic-foxy
ENV IMAGE_NAME=${IMAGE_NAME}
ENV L4T_VERSION=R35.2.1
ARG L4T_VERSION=${L4T_VERSION}
...
FROM jetson:noetic-foxy as noetic-foxy-bridge

ARG WORKSPACE=/workspace
ENV WORKSPACE=${WORKSPACE}

ARG BASE_IMAGE=lucyannofrota/jetson:noetic-foxy
ENV BASE_IMAGE=${BASE_IMAGE}
ARG IMAGE_NAME=jetson:noetic-foxy-bridge
ENV IMAGE_NAME=${IMAGE_NAME}
ENV L4T_VERSION=R35.2.1
ARG L4T_VERSION=${L4T_VERSION}
